...
use rmpv::{decode, ext::from_value, Value};

/// Tests that a `Value` is properly decoded from bytes using two different
/// mechanisms: direct deserialization using `rmp::decode::read_value` and using
/// `serde`.
...
fn pass_uint_from_value() {
    assert_eq!(
        i8::min_value(),
        from_value(Value::from(i8::min_value())).unwrap()
    );
    assert_eq!(
        i8::max_value(),
        from_value(Value::from(i8::max_value())).unwrap()
    );
    assert_eq!(
        i16::min_value(),
        from_value(Value::from(i16::min_value())).unwrap()
    );
    assert_eq!(
        i16::max_value(),
        from_value(Value::from(i16::max_value())).unwrap()
    );
    assert_eq!(
        i32::min_value(),
        from_value(Value::from(i32::min_value())).unwrap()
    );
    assert_eq!(
        i32::max_value(),
        from_value(Value::from(i32::max_value())).unwrap()
    );
    assert_eq!(
        i64::min_value(),
        from_value(Value::from(i64::min_value())).unwrap()
    );
    assert_eq!(
        i64::max_value(),
        from_value(Value::from(i64::max_value())).unwrap()
    );
}
...
fn pass_sint_from_value() {
    ...
    assert_eq!(
        u8::max_value(),
        from_value(Value::from(u8::max_value())).unwrap()
    );
    assert_eq!(
        u16::max_value(),
        from_value(Value::from(u16::max_value())).unwrap()
    );
    assert_eq!(
        u32::max_value(),
        from_value(Value::from(u32::max_value())).unwrap()
    );
    assert_eq!(
        u64::max_value(),
        from_value(Value::from(u64::max_value())).unwrap()
    );
}
...
fn pass_f32_from_value() {
    ...
    assert_eq!(
        std::f32::consts::PI,
        from_value(Value::from(std::f32::consts::PI)).unwrap()
    );
}
...
fn pass_f64_from_value() {
    ...
    assert_eq!(
        std::f64::consts::PI,
        from_value(Value::from(std::f64::consts::PI)).unwrap()
    );
}
...
fn pass_vec_from_value() {
    let v: Vec<String> =
        from_value(Value::from(vec![Value::from("John"), Value::from("Smith")])).unwrap();
