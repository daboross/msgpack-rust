...
use rmps::Serializer;
use rmpv::{encode, ext::to_value, Value};

/// Tests that a `Value` is properly encoded using two different mechanisms:
/// direct serialization using `rmp::encode::write_value` and using `serde`.
...
fn pass_str() {
    ...
    test_encode(
        Value::from("le message".to_string()),
        &[
            0xaa, 0x6c, 0x65, 0x20, 0x6d, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65,
        ],
    );
    test_encode(
        Value::from(Cow::from("le message")),
        &[
            0xaa, 0x6c, 0x65, 0x20, 0x6d, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65,
        ],
    );
...
fn pass_uint_to_value() {
    assert_eq!(
        Value::from(i8::min_value()),
        to_value(i8::min_value()).unwrap()
    );
    assert_eq!(
        Value::from(i8::max_value()),
        to_value(i8::max_value()).unwrap()
    );
    assert_eq!(
        Value::from(i16::min_value()),
        to_value(i16::min_value()).unwrap()
    );
    assert_eq!(
        Value::from(i16::max_value()),
        to_value(i16::max_value()).unwrap()
    );
    assert_eq!(
        Value::from(i32::min_value()),
        to_value(i32::min_value()).unwrap()
    );
    assert_eq!(
        Value::from(i32::max_value()),
        to_value(i32::max_value()).unwrap()
    );
    assert_eq!(
        Value::from(i64::min_value()),
        to_value(i64::min_value()).unwrap()
    );
    assert_eq!(
        Value::from(i64::max_value()),
        to_value(i64::max_value()).unwrap()
    );
}
...
fn pass_sint_to_value() {
    ...
    assert_eq!(
        Value::from(u8::max_value()),
        to_value(u8::max_value()).unwrap()
    );
    assert_eq!(
        Value::from(u16::max_value()),
        to_value(u16::max_value()).unwrap()
    );
    assert_eq!(
        Value::from(u32::max_value()),
        to_value(u32::max_value()).unwrap()
    );
    assert_eq!(
        Value::from(u64::max_value()),
        to_value(u64::max_value()).unwrap()
    );
}
...
fn pass_f32_to_value() {
    ...
    assert_eq!(
        ...
        to_value(std::f32::consts::PI).unwrap()
    );
}
...
fn pass_f64_to_value() {
    ...
    assert_eq!(
        ...
        to_value(std::f64::consts::PI).unwrap()
    );
}
...
fn pass_str_to_value() {
    ...
    assert_eq!(
        Value::from("le message"),
        to_value("le message".to_string()).unwrap()
    );
    assert_eq!(
        Value::from("le message"),
        to_value(Cow::from("le message")).unwrap()
    );
}
...
fn pass_bin_to_value() {
    assert_eq!(
        ...
        to_value(Bytes::new(&[0, 1, 2])).unwrap()
    );
    assert_eq!(
        Value::from(vec![0, 1, 2]),
        to_value(ByteBuf::from(&[0, 1, 2][..])).unwrap()
    );
}
...
fn pass_newtype_struct_to_value() {
    ...
    assert_eq!(
        Value::from("John"),
        to_value(Newtype("John".into())).unwrap()
    );
}
...
fn pass_enum_to_value() {
    ...
    assert_eq!(
        Value::Array(vec![Value::from(0), Value::Array(vec![])]),
