...
fn from_fixext16_read_fixext16() {
    let buf = [
        0xd8, 0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x01, 0x02, 0x03, 0x04, 0x05,
        0x06, 0x07, 0x08,
    ];
    let mut cur = Cursor::new(&buf[..]);
...
fn from_fixext1_read_ext_meta() {
    ...
    assert_eq!(
        ExtMeta { typeid: 1, size: 1 },
        read_ext_meta(&mut cur).unwrap()
    );
