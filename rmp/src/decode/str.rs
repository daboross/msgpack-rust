...
pub enum DecodeStringError<'a> {
    ...
    TypeMismatch(Marker),
    /// The given buffer is not large enough to accumulate the specified amount
    /// of bytes.
...
/// Attempts to read up to 9 bytes from the given reader and to decode them as a
/// string `u32` size value.
///
/// According to the MessagePack specification, the string format family stores
/// an byte array in 1, 2, 3, or 5 bytes of extra bytes in addition to the size
/// of the byte array.
///
/// # Errors
///
...
/// Attempts to read a string data from the given reader and copy it to the
/// buffer provided.
///
/// On success returns a borrowed string type, allowing to view the copied bytes
/// as properly utf-8 string.
...
///  - if the data is not utf-8, with a description as to why the provided data
///    is not utf-8 and with a size of bytes actually copied to be able to get
///    them from `out`.
///
/// # Examples
/// ```
...
/// assert_eq!(
///     "le message",
///     read_str(&mut &buf[..], &mut &mut out[..]).unwrap()
/// );
/// ```
///
/// # Unstable
...
/// Attempts to read and decode a string value from the reader, returning a
/// borrowed slice from it.
...
///
/// ```
/// use rmp::{decode::read_str_from_slice, encode::write_str};
///
/// let mut buf = Vec::new();
