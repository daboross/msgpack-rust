...
use rmp::{
    self,
    decode::{self, DecodeStringError, MarkerReadError, NumValueReadError, ValueReadError},
    Marker,
};
...
impl<R: AsRef<[u8]>> Deserializer<ReadReader<Cursor<R>>> {
    /// Returns the current position of this deserializer, i.e. how many bytes
    /// were read.
...
impl<'de, 'a, R: ReadSlice<'de>> serde::Deserializer<'de> for &'a mut Deserializer<R> {
    ...
    where
        V: Visitor<'de>,
    {
        // We need to special case this so that [] is treated as a unit struct when
        // asked for, but as a sequence otherwise. This is because we serialize
        // unit structs as [] rather than as 'nil'.
        let marker = match self.marker.take() {
            Some(marker) => marker,
            None => rmp::decode::read_marker(&mut self.rd)?,
        };
    ...
    where
        V: Visitor<'de>,
    {
        // This special case allows us to decode some integer types as floats when safe,
        // and asked for.
        //
        // This allows interoperability with msgpack-lite, which performs an
        // optimization of storing rounded floats as integers.
        let marker = match self.marker.take() {
            Some(marker) => marker,
            None => rmp::decode::read_marker(&mut self.rd)?,
        };
    ...
    {
        // This special case allows us to decode some integer types as floats when safe,
        // and asked for. This is here to be consistent with 'f32'.
        let marker = match self.marker.take() {
            Some(marker) => marker,
            None => rmp::decode::read_marker(&mut self.rd)?,
        };
...
/// Extends the `Read` trait by allowing to read slices directly by borrowing
/// bytes.
///
...
/// structure expected by `T`. It can also fail if the structure is correct but
/// `T`'s implementation of `Deserialize` decides that something is wrong with
/// the data, for example required struct fields are missing.
...
/// Deserialize an instance of type `T` from a reference I/O reader of
/// MessagePack.
///
/// Deserialization will be performed in zero-copy manner whenever it is
/// possible, borrowing the data from the reader itself. For example, strings
/// and byte-arrays won't be not copied.
///
/// # Errors
///
/// This conversion can fail if the structure of the Value does not match the
